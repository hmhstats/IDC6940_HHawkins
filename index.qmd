...
$$
\pi = P(Y=1 | X_1=x_1, X_2=x_2, \cdots, X_k=x_k)=\frac{e^{\beta_0 + \beta_1 x_1 + \cdots+\beta_kx_k}}{1 + e^{\beta_0 + \beta_1 x_1 + \cdots+\beta_kx_k}}
$$
where $\pi$ is the probability of the event occurring, $Y$ is the binary outcome (dependent) variable, $e$ is Euler’s number (approximately 2.718), the base of the natural exponential function and $X_i s$ are the predictor (independent) variables. 
[@tranmer2008binary]
[@harris2021primer]
...
Binary logistic regression models the logit transformation of the probability as a linear function of the predictors. The logit (or log-odds) is the natural logarithm of the odds of the event occurring.

$$
logit(\pi)=\ln\left(\frac{\pi}{1 - \pi}\right) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \cdots + \beta_k x_k
$$

where $\pi$ is the event probability, 
$\left(\frac{\pi}{1 - \pi}\right)$ is the odds, 
...
The logistic function converts the linear predictor back to a probability between 0 and 1:

$$
\pi=\frac{1}{1 + e^{-(\beta_0 + \beta_1 x_1 + \beta_2 x_2 + \cdots + \beta_k x_k)}}
$$

This represents the estimated probability that the event (e.g., teacher attrition) will occur given specific predictor values.
...
Data for this analysis were collected through a questionnaire administered to teachers in Northwest Florida. The survey included eight items assessing participants’ experiences with secondary traumatic stress (using items adapted from the Secondary Traumatic Stress Scale [STSS]), compassion fatigue (from the Professional Quality of Life Scale [ProQOL]), intentions to remain in or leave the teaching profession, and perceptions of whether secondary traumatic stress influenced that decision. For the purposes of this analysis, the focus is on responses to the STSS (Item 3) and participants’ stated intent to stay in or leave teaching (Item 5). In this study, teacher attrition was operationalized as any decision to leave face-to-face classroom teaching, whether through a career change, retirement, or a transition to a teaching modality (e.g., online instruction) that substantially reduces direct student interaction.
