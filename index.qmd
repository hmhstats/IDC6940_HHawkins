...
Two way contingency tables are a great way to represent categorical information. When using a two-way table, odds and odds ratios can be easily calculated. Binary logistic regression relies on odds and odds ratios (ORs) for interpretation. Odds are a ratio that compare the probability of an event happening to the probability of the event not happening. Odds ratios are calculated by dividing the odds of a certain outcome in one group by the odds of the same outcome in a comparison group.
...
Binary logistic regression relies on a few assumptions:
...
4. Any continuous predictor variables should have a linear relationship with the log odds of the dependent variable.
5. The sample size should be large. 

When the data used to create a binary logistic regression model meet these assumptions, the results are considered reliable. [@tranmer2008binary] [@harris2021primer] [@abdulqader2017applying]

RStudio will be used to conduct the statistical analyses in this project. Several R packages provide functions for performing binary logistic regression and related tasks. These include lmtest, MASS, fastDummies, tidyverse, car, and the base stats package, among others. [@zeileis2002diagnostic] [@venables2013modern] [@kaplan2020fastdummies] [@wickham2019welcome] [@fox2020using] [@team2002r] 

The goal of this project is to predict teacher attrition based on Secondary Traumatic Stress. Since teacher attrition is a binary outcome, binary logistic regression is the method that will be used for this analysis. Responses to the Secondary Traumatic Stress Scale, specifically scores for intrusion, arousal and avoidance, will be used to predict whether a teacher will stay in or leave the profession. 
